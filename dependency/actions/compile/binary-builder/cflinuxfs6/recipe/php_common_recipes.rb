...
class OdbcRecipe < FakePeclRecipe
  ...
  def setup_tar
    ...
    system <<-EOF
      cp -a -v /usr/lib/#{lib_dir}/libodbc.so* #{@php_path}/lib/
      cp -a -v /usr/lib/#{lib_dir}/libodbcinst.so* #{@php_path}/lib/
    EOF
  end
end
...
class RabbitMQRecipe < PkgConfigLibRecipe
  ...
  def compile
    execute('compile', ['bash', '-c', 'cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 .'])
    execute('compile', ['bash', '-c', 'cmake --build .'])
    execute('compile', ['bash', '-c', 'cmake -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DCMAKE_INSTALL_PREFIX=/usr/local .'])
...
class SnmpRecipe
  ...
  def cook
    system <<-EOF
      ...
      cp -Rv /etc/snmp-mibs-downloader mibs/conf
      ...
      # cp -Rv /usr/share/doc/mibiana mibs/originals
      # cp -Rv /usr/share/doc/mibrfcs mibs/originals
    EOF
  end
end
